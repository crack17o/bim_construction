...
const ModalContent = styled(motion.div)`
  ...
  width: 90%;
  max-width: 900px;
  aspect-ratio: 16/9; /* même taille pour image & vidéo */
  display: flex;
  justify-content: center;
  align-items: center;
...
const Galerie = () => {
  ...
  const videos = [
    {
      ...
      thumbnail:
        "https://res.cloudinary.com/dbwuzqvgn/image/upload/v1756481726/488199865_1061926755958969_7635251130115395931_n_ahixbb.jpg",
    },
    {
      url: "https://res.cloudinary.com/dbwuzqvgn/video/upload/v1756482126/2_OK_ix8lqa.mp4",
      thumbnail:
        "https://res.cloudinary.com/dbwuzqvgn/image/upload/v1756481722/487983933_1061643082654003_1746859014386580641_n_mqpwma.jpg",
    },
    {
      url: "https://res.cloudinary.com/dbwuzqvgn/video/upload/v1756482146/APPART_R_3_afqnfp.mp4",
      thumbnail:
        "https://res.cloudinary.com/dbwuzqvgn/image/upload/v1756481715/487849359_1061643005987344_8299947703764663113_n_xxnzvc.jpg",
    },
    {
  ...
  const prevItem = () =>
    setCurrentIndex(
      (prev) =>
        (prev - 1 + (modalType === "image" ? allPhotos.length : videos.length)) %
        (modalType === "image" ? allPhotos.length : videos.length)
    );
  const nextItem = () =>
    setCurrentIndex(
      (prev) =>
              ...
              <div className="scrolling">
                {col.concat(col).map((img, j) => (
                  <img
                    ...
                    src={img}
                    alt={`photo-${j}`}
                    onClick={() => openModal(allPhotos.indexOf(img), "image")}
                  />
                ))}
              </div>
            </Column>
